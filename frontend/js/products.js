...
// Requirements:
// - Keep function name addToCart()
// - Live search with debounce
// - Fetch GET /api/products/
// - Add to cart POST /api/cart/cart-items/ payload uses { quantity }
...
(function () {
        ...
        <div class="card product-card p-3 h-100 shadow-sm border-0" style="border-radius: 20px;">
          ...
          <div class="mt-3">
            ...
            <div class="d-flex justify-content-between align-items-center">
              ...
              <button class="btn ${btnClass} btn-sm rounded-circle ms-2" title="${btnTitle}" aria-label="${btnTitle}" style="width:38px;height:38px;display:flex;align-items:center;justify-content:center;" onclick="addToCart('${productItemId}', '${String(product.name).replace(/'/g, "\\'")}', '${price.toFixed(2)}', '${img.replace(/'/g, "\\'")}')">
                <i class='${btnIcon}'></i>
              </button>
            </div>
          </div>
  ...
  async function loadCategories() {
    ...
    // The API router exposes categories at /api/categories/ (from core router)
    // Keep it consistent with existing backend.
    try {
  ...
  window.addToCart = async function addToCart(productItemId, name) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // refresh cart state
        if (typeof window.fetchCartFromApi === 'function') {
          window.fetchCartFromApi(true); // silent refresh
        } else {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Bind badge updates only when logged in.
    const token = (() => { try { return localStorage.getItem('access_token'); } catch (_) { return null; } })();
